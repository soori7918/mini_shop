  ...
  "use strict";
    ...
    jQuery(document).ready(function(){
        ...
        */
        ...
        /* --------------------------------------------------------
            18_1. radio vandidad Slider One
        --------------------------------------------------------- */
        $('.ltn__blog-slider-one-1-active').slick({
            rtl: true,
            arrows: true,
            dots: false,
            infinite: false,
...
//chosen
    ...
    $('.checkbox-item').click(function ()
    {
        ...
        var id=$('.city_id_'+val).val();
        if(id=='all')
        {
            $('.loc_city_id').removeClass('d-none')
        }
        else
        {
            $('.loc_city_id').addClass('d-none')
            $('.loc_city_id'+id).removeClass('d-none')
        }
        $(".chosen-select").trigger("chosen:updated");
    })

    $('#sort_id').on('change',function () {
        $('.sort_id').val($(this).val())
        $('#frm_submit_search').submit()
    })

    $('#count_page').on('change',function () {
        $('.count_page').val($(this).val())
        $('#frm_submit_search').submit()
    })

    $('.a_contact_submit').click(function (){
        var name=$('#name').val()
        var email=$('#email').val()
        var message=$('#message').val()
        var check_name=false;
        var check_email=false;
        var check_message=false;
        if(name=='' || name==null)
        {
            $('#name').addClass('req_input')
            check_name=false
        }
        else
        {
            $('#name').removeClass('req_input')
            check_name=true;
        }
        if(email=='' || email==null)
        {
            $('#email').addClass('req_input')
            check_email=false
        }
        else
        {
            $('#email').removeClass('req_input')
            check_email=true;
        }
        ...
        if(check_name==true && check_email==true && check_message==true)
        {
            $('#frm_contact_submit').submit()
        }
    })
    $('.a_contact_submit1').click(function (){
        var name=$('#name').val()
        var city=$('#city').val()
        var login_tr=$('#login_tr').val()
        var along_count=$('#along_count').val()
        var time_tr=$('#time_tr').val()
        var know_izmir=$('#know_izmir').val()
        ...
        var check_login_tr=false;
        var check_along_count=false;
        var check_time_tr=false;
        var check_know_izmir=false;
        var check_target_tr=false;
        var check_start_price=false;
        var check_email=false;
        ...
        if(city=='' || city==null)
        {
            $('#city').addClass('req_input')
            check_city=false
        }
        else
        {
            $('#city').removeClass('req_input')
            check_city=true;
        }
        if(login_tr=='' || login_tr==null)
        {
            $('#login_tr').addClass('req_input')
            check_login_tr=false
        }
        else
        {
            $('#login_tr').removeClass('req_input')
            check_login_tr=true;
        }
        if(along_count=='' || along_count==null)
        {
            $('#along_count').addClass('req_input')
            check_along_count=false
        }
        else
        {
            $('#along_count').removeClass('req_input')
            check_along_count=true;
        }
        if(time_tr=='' || time_tr==null)
        {
            $('#time_tr').addClass('req_input')
            check_time_tr=false
        }
        else
        {
            $('#time_tr').removeClass('req_input')
            check_time_tr=true;
        }
        if(know_izmir=='' || know_izmir==null)
        {
            $('#know_izmir').addClass('req_input')
            check_know_izmir=false
        }
        else
        {
            $('#know_izmir').removeClass('req_input')
            check_know_izmir=true;
        }
        if(target_tr=='' || target_tr==null)
        {
            $('.input-item-target .nice-select').addClass('req_input')
            check_target_tr=false
        }
        else
        {
            $('.input-item-target .nice-select').removeClass('req_input')
            check_target_tr=true;
        }
        if(start_price=='' || start_price==null)
        {
            $('#start_price').addClass('req_input')
            check_start_price=false
        }
        else
        {
            $('#start_price').removeClass('req_input')
            check_start_price=true;
        }
        if(email=='' || email==null)
        {
            $('#email').addClass('req_input')
        ...
        if(phone=='' || phone==null)
        {
            $('#phone').addClass('req_input')
            check_phone=false
        }
        else
        {
            $('#phone').removeClass('req_input')
            check_phone=true;
        }
        // if(message=='' || message==null)
        // {
        ...
        //     check_message=true;
        // }
        if(check_name==true && check_city==true && check_login_tr==true &&
            check_along_count==true && check_time_tr==true && check_know_izmir==true &&
            check_target_tr==true && check_start_price==true && check_email==true && check_phone==true)
        {
            $('#frm_contact_submit1').submit()
        }
    })
    $('.select_price_type').click(function (){
        var val=$(this).attr('data-val')
        var type=$(this).attr('data-type')
        var btn=$(this).attr('data-btn')
        var price=$(this).attr('data-price')
        var d_none=$(this).attr('data-this')
...
$('.price_type_select').on('change',function (){
    ...
    if(val_s=='price_lir')
    {
        $('.lir-rang').removeClass('d-none')
        $('.toman-rang').addClass('d-none')
        $('.dollar-rang').addClass('d-none')
    }
    else if(val_s=='price_toman')
    {
        $('.lir-rang').addClass('d-none')
        $('.toman-rang').removeClass('d-none')
        $('.dollar-rang').addClass('d-none')
    }
    else if(val_s=='price_dollar')
    {
        $('.lir-rang').addClass('d-none')
        $('.toman-rang').addClass('d-none')
        $('.dollar-rang').removeClass('d-none')
    }
})
...
input.addEventListener("keyup", function(event) {
    if (event.keyCode === 13) {
        event.preventDefault();
        document.getElementById("search_btn_header").click();
    }
});
var input = document.getElementById("search_input_header1");
input.addEventListener("keyup", function(event) {
    if (event.keyCode === 13) {
        event.preventDefault();
        document.getElementById("search_btn_header1").click();
    }
});
